...
| tee ファイル... | 標準入力を標準出力と1つ以上のファイルに同時に出力する      |
| :               | 常に終了ステータス0を返す。while :で無限ループとなる       |
| time コマンド   | コマンドにかかった時間を表示する                           |
...
| groups               | 所属するグループを表示                     |
| cmp                  | ファイルの内容を1バイトずつ比較する        |
| comm                 | ソート済みの2つのファイルをマッチングする  |
| basename パス        | パスからファイル名を取得する               |
| basename パス 接尾辞 | パスからファイル名を取得し接尾辞を取り除く |
| dirname パス         | パスからディレクトリ名を取得する           |
| file                 | ファイルの種類を表示                       |
...
| cat -n                   | 行番号を表示                                        |
| join -1 列番号 -2 列番号 | 2つのファイルを結合する。デフォルトキーは左端の項目 |
| wc                       | 行数、単語数、バイト数を表示する                    |
| wc -l                    | 行数を表示                                          |
| wc -w                    | 単語数を表示                                        |
| wc -c                    | バイト数を表示                                      |
...
| sort -u                  | 重複行を1行だけ表示する                             |
| uniq -c                  | 重複行の行数を表示する                              |
| cut -d , -f 1,3-5,7      | ,で区切り、1、3から5、7番目のフィールドを切り出す   |
| tr abc ABC               | aをA、bをB、cをCに置き換える                        |
| tr a-c A-C               | aをA、bをB、cをCに置き換える                        |
| tr -d "\n"               | LFを削除する                                        |
| tr -c abc X              | abc以外の文字をXに置き換える（引数を反転する）      |
| tr -s                    | 2文字以上連続する場合に1文字に置き換える            |
| fmt                      | 行の改行位置を変更して段落を読みやすくする          |
| fmt -s                   | 短い行を連結しない                                  |
| fmt -w 文字数            | 行の長さを指定する                                  |
...
| C--                           | undo                                                   |
| C-u                           | 行削除                                                 |
| Esc q                         | カレント行を退避してクリアし、実行後に自動入力する     |
| history                       | 履歴を16件表示                                         |
| history 履歴番号              | 履歴番号以降の履歴を表示                               |
| hash -d エイリアス=パス       | ~エイリアスでパスを指定できる                          |
| fc                            | 直前のコマンドをエディタで編集して実行                 |
| fc 履歴番号 履歴番号          | 履歴番号から履歴番号の履歴をエディタで編集して実行     |
| fc コマンド コマンド          | コマンドからコマンドまでの履歴をエディタで編集して実行 |
| fc 文字列1=文字列2            | 文字列1を2に置換してから履歴をエディタで編集して実行   |
| ~/p/k tab                     | ~/projects/know-howに展開                              |
| cd - tab                      | 移動履歴を表示                                         |
| Esc .                         | 直前に入力した単語を入力                               |
| $ tab                         | 変数名を補完する                                       |
| !コマンド名                   | コマンドを補完                                         |
| !!                            | 直前のコマンドを入力                                   |
| !:1                           | 直前のコマンドの第一引数を入力                         |
| !:1-3                         | 直前のコマンドの第一〜第三引数を入力                   |
| !:$                           | 直前のコマンドの最後の引数を入力                       |
| !:*                           | 直前のコマンドの全ての引数を入力                       |
| !:0                           | 直前のコマンドのコマンドのみを入力                     |
| =コマンド                     | コマンドの絶対パスを入力(\`which コマンド\`と同様)     |
| {文字列1,文字列2,文字列3}.txt | 文字列1.txt、文字列2.txt、文字列3.txtに展開            |
| {1..5}.txt                    | 1.txt〜5.txtに展開                                     |
...
| \*\*/                       | サブディレクトリ内を再帰的に検索                             |
| (文字列｜文字列)            | OR条件で検索                                                 |
| ^グロブ文字列               | グロブ文字列に一致しないファイルを検索                       |
| グロブ文字列1~グロブ文字列2 | グロブ文字列1に一致しグロブ文字列2に一致しないファイルを検索 |
| \<10-19\>                   | 10から19を検索                                               |
| (.)                         | 通常ファイルを検索                                           |
| (/)                         | ディレクトリを検索                                           |
| (*)                         | 実行ファイルを検索                                           |
| (@)                         | シンボリックリンクを検索                                     |
| (L数値)                     | ファイルサイズが数値のファイルを検索                         |
| (L+数値)                    | ファイルサイズが数値より大きいファイルを検索                 |
| (L-数値)                    | ファイルサイズが数値より小さいファイルを検索                 |
| (u:ユーザ名:)               | 所有者がユーザ名のファイルを検索                             |
| (uUID)                      | 所有者がUIDのファイルを検索                                  |
| (g:グループ名:)             | グループがグループ名のファイルを検索                         |
| (gGID)                      | グループがGIDのファイルを検索                                |
| (^@*)                       | シンボリックリンクと実行ファイル以外を検索                   |
| グロブ文字列 tab            | グロブが展開される                                           |
...
|:----------------------|:-----------------------------------------|
| screen                | screenを起動                             |
| C-t                   | screenのコマンドキーC-aをC-tにマッピング |
| C-t w                 | ウィンドウ一覧を表示                     |
| C-t c                 | 新規ウィンドウを作成                     |
| C-t A                 | ウィンドウ名を変更                       |
| C-t 0                 | 1番目のウィンドウを開く                  |
...
| C-t t                 | 直前のウィンドウを開く                   |
| C-t S                 | ウィンドウを水平分割する                 |
| C-t tab               | 別のウィンドウに移動する                 |
| C-t X                 | カレントウィンドウを閉じる               |
| C-t Q                 | カレントウィンドウ以外を閉じる           |
| C-t m                 | 監視モードに入る                         |
| C-t [                 | コピーモードに入る                       |
| (コピーモード)Space   | コピー範囲を選択する                     |
| (コピーモード)Esc     | コピーモードから抜ける                   |
| (別ウィンドウ等)C-t ] | ペーストする                             |
| C-t d                 | デタッチする                             |
| screen -r             | アタッチする                             |
| screen -rx            | 他の端末からアタッチする                 |
| screen -ls            | デタッチ中のscreen一覧を表示             |
